---
title: Codex Runtime
description: How to run Hive Mind patterns using the OpenAI Codex CLI as the execution engine. Covers agent model differences, pattern adaptations, handoff contracts, the run ledger, and prompt templates for each role.
---

# Codex Runtime

Codex is an alternative runtime for the same four patterns (Patchwork, Worker
...
This document covers the differences, the adaptations, and the templates you
need to run any of the four patterns inside a Codex session.
...
definitions and the decision matrix for choosing between them are in the
...
- [Patchwork](./patchwork.md): single-agent baseline
- [Worker Swarm](./worker-swarm.md): lead-directed parallel agents
- [Research Swarm](./research-swarm.md): scan-driven discovery
- [Overview](./overview.md): decision matrix for all patterns
